...
const MechMasterRecommendationInputSchema = z.object({
  manufacturingProcess: z
    .string()
    .describe(
      'The primary manufacturing process required (e.g., CNC Milling/Turning, Laser Cutting).'
    ),
  material: z
    .string()
    .describe(
      'The material required for manufacturing (e.g., Stainless Steel, Aluminum, ABS Plastic).'
    ),
  ...
  surfaceFinish: z
    ...
    .optional()
    .describe(
      'Optional: Specific surface finish requirements (e.g., Polished, Anodized, Powder Coated).'
    ),
  toleranceRequirement: z
    .string()
    .optional()
    .describe('Optional: Specific tolerance requirements (e.g., +/- 0.05mm, ISO 2768-m).'),
  requiredDeliveryDate: z
    .string()
    .describe('The required delivery date for the manufactured parts (YYYY-MM-DD format).'),
  budgetRange: z
    .string()
    .optional()
    .describe('Optional: The estimated budget range for the project (e.g., $1000-$5000).'),
  deliveryLocation: z
    .string()
    .describe('The city and pincode for delivery (e.g., "Mumbai, 400001").'),
  additionalNotes: z
    .string()
    .optional()
    .describe('Any additional notes or specific requirements from the user.'),
});
...
const MechMasterRecommendationOutputSchema = z.object({
  processFilter: z
    .string()
    .describe('The most suitable manufacturing process identified from the request.'),
  materialFilter: z.string().describe('The most suitable material identified from the request.'),
  locationFilter: z
    .string()
    .describe(
      'The primary geographic location (city or region) identified for filtering MechMasters.'
    ),
  pincodeFilter: z
    .string()
    .optional()
    .describe('The pincode identified for filtering MechMasters, if highly specific.'),
  geographicImportance: z
    .boolean()
    .describe(
      'True if geographic proximity or specific local presence is critical for this request (e.g., on-site work, highly localized delivery, urgent local pickup).'
    ),
  recommendationRationale: z
    .string()
    .describe(
      'A detailed explanation of why these filters were chosen, highlighting any implicit geographic preferences or critical aspects of the request.'
    ),
});
export type MechMasterRecommendationOutput = z.infer<typeof MechMasterRecommendationOutputSchema>;

export async function mechMasterSmartRecommendation(
  input: MechMasterRecommendationInput
): Promise<MechMasterRecommendationOutput> {
