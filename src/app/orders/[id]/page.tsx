    ...
    if (error || !order) {
        return (
            <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
                ...
                <p className="text-slate-500 mb-8 text-center max-w-md">The order record could not be retrieved from the central procurement system.</p>
                <Link href="/shop"><Button variant="outline" className="border-slate-200">Back to Shop</Button></Link>
            </div>
        );
    }
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                    <div className="space-y-2">
                        ...
                        <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest pl-1 mt-1">
                            Ref ID: <span className="font-mono text-[#2F5FA7]">#{order.id?.slice(-8).toUpperCase() || 'UNKNOWN'}</span>
                        </p>
                    </div>
                                ...
                                <div className="divide-y divide-slate-100">
                                    {(order.items || []).map((item: any) => (
                                        <div key={item.id} className="py-6 flex justify-between items-start gap-6 group">
                                            <div className="flex-1 min-w-0">
                                                <p className="text-base font-bold text-slate-900 transition-colors leading-tight uppercase tracking-tight">{item.name}</p>
                                                <div className="flex items-center gap-3 mt-2">
                                                    <Badge variant="outline" className="text-[9px] border-slate-100 text-slate-500 font-bold uppercase tracking-widest py-0 px-2 h-5 bg-slate-50">SKU: {item.sku}</Badge>
                                                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Qty: <span className="text-[#2F5FA7] font-mono leading-none">{item.quantity}</span></span>
                                                </div>
                                            </div>
                                            <div className="text-right shrink-0">
                                                <p className="text-sm font-bold font-mono text-slate-900 italic">₹{(item.quantity * item.price).toLocaleString()}</p>
                                                <p className="text-[9px] text-slate-400 mt-1 uppercase tracking-widest font-bold">₹{item.price?.toLocaleString() || '0'} unit</p>
                                            </div>
                                        </div>
                                    ))}
                                    ...
                                    {(order.shopItems || []).map((item: any) => (
                                        <div key={item.id} className="py-6 flex justify-between items-start gap-6 group">
                                            ...
                                            <div className="text-right shrink-0">
                                                <p className="text-sm font-bold font-mono text-slate-900 italic">₹{(item.quantity * item.salePrice).toLocaleString()}</p>
                                                <p className="text-[9px] text-slate-400 mt-1 uppercase tracking-widest font-bold">₹{item.salePrice?.toLocaleString() || '0'} unit</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <Card className="bg-white border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                                ...
                                <CardContent className="p-6 pt-2">
                                    <div className="space-y-1">
                                        <p className="text-sm font-bold text-slate-900 uppercase tracking-tight">{order.shippingAddress?.fullName || 'N/A'}</p>
                                        <p className="text-xs text-slate-500 font-bold uppercase tracking-wider leading-relaxed">
                                            {order.shippingAddress?.street || order.shippingAddress?.addressLine1}, {order.shippingAddress?.city}<br />
                                            {order.shippingAddress?.state} - {order.shippingAddress?.pincode}<br />
                                            <span className="text-slate-400 font-medium">Contact:</span> <span className="text-[#2F5FA7] font-mono">{order.shippingAddress?.phone}</span>
                                        </p>
                                    </div>
                                </CardContent>
                            </Card>
                                ...
                                <CardContent className="p-6 pt-2">
                                    <div className="space-y-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">QC Passed</span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                                            <span className="text-[10px] font-bold text-slate-600 uppercase tracking-wider">Tax Invoice Generated</span>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                    ...
                    <div className="lg:col-span-1 space-y-6">
                            ...
                            <CardContent className="p-8 pt-6 space-y-6">
                                <div className="space-y-4">
                                    <div className="flex justify-between text-[11px] font-bold uppercase tracking-widest text-blue-100">
                                        ...
                                        <span className="font-mono text-sm leading-none">₹{(order.pricing?.subtotal || 0).toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between text-[11px] font-bold uppercase tracking-widest text-blue-100">
                                        <span>GST (IGST 18%)</span>
                                        <span className="font-mono text-sm leading-none">₹{(order.pricing?.gst || 0).toLocaleString()}</span>
                                    </div>
                                    <div className="flex justify-between text-[11px] font-bold uppercase tracking-widest text-blue-100">
                                        <span>Procure Shipping</span>
                                        <span className="font-mono text-sm leading-none">{order.pricing?.shipping === 0 ? 'FREE' : `₹${(order.pricing?.shipping || 0).toLocaleString()}`}</span>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center py-1">
                                        <span className="text-blue-100 font-bold uppercase text-[10px] tracking-widest">Gross Total</span>
                                        <span className="text-3xl font-bold text-white font-mono tracking-tighter italic">₹{(order.pricing?.total || 0).toLocaleString()}</span>
                                    </div>
                                    <Badge className={`${isPaid ? 'bg-emerald-500 text-white border-transparent' : 'bg-orange-500 text-white border-transparent'} w-full flex justify-center py-2.5 rounded-xl text-[10px] uppercase font-bold tracking-[0.2em] shadow-lg`}>
                                ...
                                <div className="space-y-4 pt-2">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-[9px] text-blue-200 font-bold uppercase tracking-widest">Procured Date</p>
                                            <p className="text-xs font-bold text-white italic">{new Date(order.createdAt).toLocaleDateString()}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                                        <div className="w-9 h-9 rounded-xl bg-white/10 flex items-center justify-center shrink-0 border border-white/5">
                                        ...
                                        <div>
                                            <p className="text-[9px] text-blue-200 font-bold uppercase tracking-widest">Fulfillment Type</p>
                        ...
                        <div className="p-6 rounded-3xl bg-white border border-slate-100 shadow-sm flex items-center justify-between group cursor-pointer hover:bg-slate-50 transition-all">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">View Tax Invoice</span>
                            </div>
