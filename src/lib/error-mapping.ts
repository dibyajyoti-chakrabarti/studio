...
export function resolveUserFriendlyMessage(error: any): UserFriendlyMessage {
    ...
    const defaultDescription = "An unexpected error occurred. Please try again or contact support if the issue persists.";
    const defaultVariant: 'default' | 'destructive' = 'destructive';

    if (!error) return { title: defaultTitle, description: defaultDescription, variant: defaultVariant };
        ...
        switch (error.code) {
            ...
            case 'auth/user-not-found':
                return {
                    ...
                    description: "No account exists with this email address. Please verify the spelling or register for a new account.",
                    variant: 'destructive'
                };
            case 'auth/email-already-in-use':
                return {
                    title: "Email already exists",
                    description: "This email address is already registered. Try signing in or resetting your password.",
                    variant: 'destructive'
                };
            case 'auth/weak-password':
                return {
                    title: "Password too weak",
                    description: "For your security, please choose a stronger password with at least 8 characters.",
                    variant: 'destructive'
                };
            case 'auth/too-many-requests':
                return {
                    title: "Account temporarily locked",
                    description: "Too many failed attempts. Please try again in a few minutes or reset your password.",
                    variant: 'destructive'
                };
            case 'auth/popup-closed-by-user':
            ...
            case 'auth/permission-denied':
                return {
                    ...
                    description: "You don't have secondary permissions to perform this action. Please sign in with an authorized account.",
                    variant: 'destructive'
                };
        }
    }
    ...
    if (error.code && typeof error.code === 'string') {
        ...
        if (code === 'INTERNAL_ERROR') {
            return {
                title: "Server Error",
                description: "Our systems are experiencing a temporary issue. Please try again in 1-2 minutes.",
                variant: 'destructive'
            };
        }
    }
