...
function checkEdgeToHoleDistance(
  geometry: ParsedGeometry,
  thicknessMm: number
): DFMIssue[] {
...
function checkOpenGeometry(geometry: ParsedGeometry): DFMIssue[] {
  if (geometry.hasOpenPaths) {
    return [{
      code: 'OPEN_GEOMETRY',
      severity: 'error',
      message: 'File contains unclosed geometry. All cut paths must form closed loops for laser cutting.',
