...
export function ServiceMaterialGrid({ 
  ...
  const filteredMaterials = useMemo(() => {
    return serviceMaterials.filter(mat => {
      ...
      <div className="container mx-auto px-4 md:px-10 lg:px-20 relative z-10">
          ...
          {filteredMaterials.map((mat) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm font-black text-slate-900 mb-1 truncate uppercase tracking-tight group-hover:text-blue-600 transition-colors">{mat.name}</div>
                <div className="text-[10px] font-bold text-slate-500 truncate opacity-80">{mat.thicknesses}</div>
              </div>
        ...
        {filteredMaterials.length === 0 && (
          <div className="text-center py-20 bg-slate-50 rounded-[3rem] border border-dashed border-slate-200">
            ...
            <p className="text-slate-500 font-bold uppercase tracking-widest">No materials found for this search</p>
          </div>
        )}
      </div>
          ...
          <div className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white z-[101] shadow-2xl animate-in slide-in-from-right duration-500 overflow-y-auto">
            <div className="p-8">
              ...
              <div className="space-y-8">
                ...
                <div>
                  <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">SPECIFICATIONS</h4>
                  <div className="grid grid-cols-2 gap-4">
                    {[{ l: 'Thickness', v: selectedMaterial.thicknesses }, { l: 'Lead Time', v: '3-5 Days' }, { l: 'Sourcing', v: 'Industrial' }, { l: 'Quality', v: 'Verified' }].map(p => (
                ...
                <div>
                  <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">COMPATIBILITY</h4>
                  <div className="flex flex-wrap gap-2">
                    {selectedMaterial.processes.map((proc: string) => (
                      <div key={proc} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black uppercase text-slate-600">
                ...
                <div className="pt-10">
                  ...
                  >
                    Get Quote with this Material <ChevronRight className="ml-2 w-5 h-5 group-hover:translate-x-1 transition-transform" />
                  </Button>
                </div>
              </div>
            </div>
