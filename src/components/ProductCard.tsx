...
export function ProductCard({ product, isComparing, toggleCompare, addItem }: ProductCardProps) {
    ...
    return (
        ...
        >
            <Card className={`flex-1 bg-[#0B1120] border-white/[0.06] overflow-hidden hover:border-cyan-500/30 transition-all duration-300 rounded-xl md:rounded-2xl flex flex-col shadow-lg ${isOutOfStock ? 'opacity-90 grayscale-[0.3]' : ''}`}>
                {/* Image Section */}
                <Link href={`/shop/${product.id}`} className="block">
                    <div className="relative aspect-square sm:aspect-[4/3] md:aspect-square bg-[#0F172A] flex items-center justify-center p-3 sm:p-6 overflow-hidden border-b border-white/[0.04]">
                        ...
                        <div className={`relative w-full h-full transform group-hover:scale-105 transition-transform duration-700 ease-out ${isOutOfStock ? 'opacity-50' : ''}`}>
                            <Image
                                ...
                                onError={(e: any) => { e.target.src = "/mechhub.png"; }}
                            />
                        </div>
                        ...
                        {!isOutOfStock && (
                            <button
                                ...
                                className={`absolute top-2 right-2 p-1.5 rounded-lg border transition-all z-30 ${isComparing
                ...
                <div className="p-3 md:p-5 flex flex-col flex-1 gap-1.5 md:gap-3">
                    ...
                    <Link href={`/shop/${product.id}`} className="block space-y-1 md:space-y-2">
                        <div className="flex items-center gap-2">
                            <span className="text-[7px] md:text-[9px] font-mono text-cyan-500 bg-cyan-500/10 border border-cyan-500/20 px-1 py-0.5 rounded uppercase font-bold tracking-tighter">{product.sku}</span>
                            {!isOutOfStock && product.inventory < 20 && <Badge className="bg-orange-500/10 text-orange-400 border-none text-[7px] font-bold px-1 py-0">Limited</Badge>}
                        </div>
                    ...
                    <div className="flex items-center gap-1">
                        <div className="flex text-yellow-500">
                            {[...Array(5)].map((_, i) => (
                                <Star key={i} className={`w-2.5 h-2.5 md:w-3.5 md:h-3.5 ${i < Math.floor(avgRating) ? 'fill-current' : 'text-zinc-700'}`} />
                            ))}
                        </div>
                        <span className="text-[8px] md:text-[10px] text-zinc-500 font-bold">({reviewCount})</span>
                    </div>

                    {/* Price Section */}
                    <div className="space-y-0.5">
                        <div className="flex items-center gap-1.5 md:gap-2">
                            <span className={`text-lg md:text-2xl font-black font-mono ${isOutOfStock ? 'text-zinc-500' : 'text-white'}`}>₹{product.salePrice?.toLocaleString()}</span>
                            <span className="text-[9px] md:text-[11px] text-zinc-600 line-through">₹{product.basePrice || product.salePrice + 100}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Badge className={`${isOutOfStock ? 'bg-zinc-500/10 text-zinc-500' : 'bg-emerald-500/10 text-emerald-400'} border-none px-1 py-0 text-[8px] md:text-[10px] font-black`}>
                                {Math.round(((product.basePrice - product.salePrice) / product.basePrice) * 100) || 15}% OFF
                            </Badge>
                        </div>
                    </div>
                    ...
                    <div className="text-[8px] md:text-[10px] text-zinc-500 font-medium">
                        {isOutOfStock ? 'Currently unavailable' : <>FREE Delivery <span className="text-zinc-400 font-bold">Tomorrow</span></>}
                    </div>
                    ...
                    <div className="mt-2 md:mt-auto">
                        {isOutOfStock ? (
                            ...
                            >
                                {isRequesting ? 'Requesting...' : hasRequested ? 'Request Sent ✓' : 'Notify Me (Priority)'}
                            </Button>
                        ) : (
                            <Button
