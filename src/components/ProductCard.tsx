...
export function ProductCard({ product, isComparing, toggleCompare, addItem }: ProductCardProps) {
    ...
    return (
        ...
        >
            <Card className="flex-1 bg-[#0B1120] border-white/[0.06] overflow-hidden hover:border-cyan-500/30 transition-all duration-300 rounded-xl md:rounded-2xl flex flex-col shadow-lg">
                {/* Image Section */}
                ...
                <div className="p-3 md:p-5 flex flex-col flex-1 gap-1.5 md:gap-3">
                    ...
                    <Link href={`/shop/${product.id}`} className="block space-y-1 md:space-y-2">
                        <div className="flex items-center gap-2">
                            ...
                            {product.inventory < 20 && <Badge className="bg-orange-500/10 text-orange-400 border-none text-[7px] font-bold px-1 py-0">Limited</Badge>}
                        </div>
                    ...
                    <div className="space-y-0.5">
                        <div className="flex items-center gap-1.5 md:gap-2">
                            <span className="text-lg md:text-2xl font-black font-mono text-white">₹{product.salePrice?.toLocaleString()}</span>
                            <span className="text-[9px] md:text-[11px] text-zinc-600 line-through">₹{product.basePrice || product.salePrice + 100}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Badge className="bg-emerald-500/10 text-emerald-400 border-none px-1 py-0 text-[8px] md:text-[10px] font-black">
