...
const components = {
  ...
  ul: ({ className, ...props }: any) => (
    <ul
      className={cn('my-8 ml-6 list-disc space-y-3 text-slate-600 leading-relaxed', className)}
      {...props}
    />
  ),
  ol: ({ className, ...props }: any) => (
    <ol
      className={cn('my-8 ml-6 list-decimal space-y-3 text-slate-600 leading-relaxed', className)}
      {...props}
    />
  ),
  ...
  img: ({ className, alt, ...props }: React.ImgHTMLAttributes<HTMLImageElement>) => (
    ...
    <img
      className={cn('rounded-xl border border-slate-200 my-8', className)}
      alt={alt}
      {...props}
    />
  ),
